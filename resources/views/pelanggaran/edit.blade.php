<x-layout.default>
    ...
    <div>
        <form action="{{ url('pelanggaran/' . $data->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                        ...
                        <div class="grid grid-cols-1 gap-4">
                            ...
                            <div>
                                ...
                                <select required id="siswa_id" name="siswa_id" class="form-select w-full">
                                    ...
                                    @foreach($siswa as $item)
                                        <option value="{{ $item->id }}" @selected($data->siswa_id == $item->id)>{{ $item->nama }}</option>
                                    @endforeach
                                </select>
                                @error('siswa_id')
                                    <div class="mt-2 text-danger">{{ $message }}</div>
                            ...
                            <div>
                                ...
                                <textarea id="pelanggaran" rows="5" name="pelanggaran" class="form-textarea" 
                                    placeholder="Isi Pelanggaran" required>{{ $data->pelanggaran }}</textarea>
                                @error('pelanggaran')
                                    <div class="mt-2 text-danger">{{ $message }}</div>
                                @enderror
                            </div>
                            <div>
                                ...
                                <textarea id="solusi" rows="5" name="solusi" class="form-textarea" 
                                    placeholder="Isi Solusi" required>{{ $data->solusi }}</textarea>
                                @error('solusi')
                                    <div class="mt-2 text-danger">{{ $message }}</div>
                                @enderror
                            </div>
        ...
        document.addEventListener("alpine:init", () => {
            Alpine.data("tanggal", () => ({
                ...
                init() {
                    ...
                    this.date1 = this.date1  ? this.date1 : today.toLocaleDateString('id-ID', options);
